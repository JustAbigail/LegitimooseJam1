...
execute if entity @e[distance=..0.2,predicate=jam:vulnerable_to_thrown_blocks] run tag @s add block.thrown.trace.hit
execute as @e[distance=..0.2,limit=1,sort=nearest] if predicate jam:vulnerable_to_thrown_blocks at @s run return run function jam:player/throw/hit_entity
